...
**Status**: Implementation in progress. Themes and the new browser commands are not
...
## Baseline record (T001)

- Baseline revision: `d4ae026` (main after PR #16/#17 merged; pre-refactor generator).
- Numeric fixtures: `tests/fixtures/alien.ts`, regenerated by
  `npx vite-node scripts/capture-alien-baseline.ts <revision>`; asserted by
  `tests/sim/alien-baseline.test.ts`.
- Viewpoints for the matching PR captures (page `/?seed=42`, 960x600, SwiftShader):
  - `alien-baseline-flight.png`: ~4 s after load, default controls — plane heading +z over the
    spawn region, Pastel Dawn sky and low sun visible.
  - `alien-baseline-overview.png`: same flight ~29 s in, after wheel-up throttle and a held
    nose-up pointer (climb), giving a raised vantage over the same terrain.
...
## Browser and soak commands (added in T002/T056)

The implementation adds `playwright@1.63.0` as an exact-pinned development dependency, locked in
`package-lock.json`. The migrated real-render smoke lives at `tests/render/webgl.browser.test.ts`.
...
`test:browser` owns an isolated verification build in `.verify-dist/` (never the production
`dist/`), a free-port preview server, the serial Vitest suite (`vitest.browser.config.ts`),
browser, and cleanup. The default `npm test` continues to run Node mechanics without launching
a browser. CI installs Chromium and runs the browser suite after the existing checks;
unavailable WebGL2 is a failed environment check, not a skipped green smoke test.
...
| Theme | Required visible cues | Approved reference |
|---|---|---|
| Nature | Gentler Earth-like relief, green foothills/forest bands, natural rock, white snow, blue lakes, clear daylight. | N4 Limestone Lakes flight/overview captures (Seed 42), [spec references](spec.md#approved-visual-references) |
| Alien Planet | Existing Alpine/Foothills shape, cool dusky terrain, warm gold lakes, Pastel Dawn sky, low pale-gold sun, lavender fog. No new spires/craters/objects. | T001 baseline captures (flight/overview at Seed 42) |
| Arctic | Broad glacial valleys, snowy ridges, blue ice and flat frozen lakes, cold daylight; no temperate forest look. | A4 Blue Glacier Basin flight/overview captures (Seed 42), [spec references](spec.md#approved-visual-references) |
